...
def getWordID(Value,SentenceId):
    '''returns ID for a particular sentence'''
    position = wordIDs[SentenceId].values().index(Value)
    return wordIDs[SentenceId].keys()[position]
...
for line in sentences:
    ...
    for word in tokens:
        ...
        wIDCounter += 1
        wordIDs[sentkey][wordkey] = word #add the word id and word to the dictionary of word IDs
...
for id in sentenceIDs:
    ...
    for word in tokens:
        ...
        if wordIndex < ntokens-1:
            nextWordID = getWordID(tokens[wordIndex+1],id)
        ...
        '''if wordIndex > 0:
            prevWordID = getWordID(tokens[wordIndex-1],id)
        ...
        '''if wordIndex < ntokens-2:
            nextOfNextWordID = getWordID(tokens[wordIndex+2],id)
        ...
        '''if wordIndex > 1:
            prevOfPrevWordID = getWordID(tokens[wordIndex-2],id)
